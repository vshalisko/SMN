...
```{r}
files <- list.files(path = "./Base", pattern = "^dia14.*\\.txt$")
files
```
...
```{r}
write.csv(smn_data_df,"smn_14_datos_diarios.csv", row.names = FALSE)
```
...
st_write(smn_points_sf, "smn_14_puntos_occidente.shp")
```
