...
```{r}
files <- list.files(path = "../Base_diarios_TXT", pattern = "^dia18080.*\\.txt$")
files
file_name <- paste0("../Base_diarios_TXT/",files[1])
```
...
```{r}
datos_corto <- smn_data_df[smn_data_df$ESTACION == 18080
...
```{r}
datos_1003 <- smn_data_df[smn_data_df$ESTACION == 18080
                          & smn_data_df$FECHA >= as.Date("1970-01-01"), ]
dim(datos_1003)

t_mens_1003 <- promedios_mensuales_t(datos_1003)

head(t_mens_1003)
plot(x=as.numeric(t_mens_1003$M),y=t_mens_1003$TMIN, col="blue")
plot(x=as.numeric(t_mens_1003$M),y=t_mens_1003$TMAX, col="red")

p_mens_1003 <- sumas_mensuales_p(datos_1003)

head(p_mens_1003)
plot(x=as.numeric(p_mens_1003$M),y=p_mens_1003$PREC, col="black")
plot(x=as.numeric(p_mens_1003$M),y=p_mens_1003$EVAP, col="darkgreen")

t_mens_locales_1003 <- promedios_locales_t(datos_1003)
head(t_mens_locales_1003)

plot(x=t_mens_locales_1003$FECHA,y=t_mens_locales_1003$TMIN, col="blue", type="l")
plot(x=t_mens_locales_1003$FECHA,y=t_mens_locales_1003$TMAX, col="red", type="l")

plot(x=as.numeric(t_mens_locales_1003$D),y=t_mens_locales_1003$TMIN, col="blue", type="p", cex=0.5)
plot(x=as.numeric(t_mens_locales_1003$D),y=t_mens_locales_1003$TMAX, col="red", type="p", cex=0.5)
```
